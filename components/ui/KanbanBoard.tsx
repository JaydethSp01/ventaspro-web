                ...
                <div className="mt-2 flex items-center justify-between">
                  ...
                  <div className="flex gap-1">
                    <button onClick={() => move(d.id, -1)} disabled={si === 0}
                      className="rounded-md border border-slate-200 px-1.5 text-slate-500 hover:bg-slate-50 disabled:opacity-30 cursor-pointer" aria-label="Atrás">←</button>
                    <button onClick={() => move(d.id, 1)} disabled={si === stages?.length - 1}
                      className="rounded-md border border-slate-200 px-1.5 text-slate-500 hover:bg-slate-50 disabled:opacity-30 cursor-pointer" aria-label="Avanzar">→</button>
                  </div>
                </div>
              </div>
            ))}
